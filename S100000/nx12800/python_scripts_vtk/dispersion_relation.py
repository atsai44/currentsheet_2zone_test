...
omega = np.fft.fftshift(np.fft.fftfreq(len(t_physical), d = delta_t_physical))  # normalized frequency
k = np.fft.fftshift(np.fft.fftfreq(len(x/L), d = delta_x))              # normalized wavenumber

# fold negative and positive k and omega
v_tilde_sqr = v_tilde_sqr + v_tilde_sqr[::-1, :]   # fold negative omega
v_tilde_sqr = v_tilde_sqr + v_tilde_sqr[:, ::-1]   # fold negative k
